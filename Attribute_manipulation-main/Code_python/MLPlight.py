...
if __name__ == '__main__':
  mlp = MLP()
  trainer = pl.Trainer(gpus=0, deterministic=True, max_epochs=50)
  trainer.fit(mlp, DataLoader(dataset,batch_size=64,num_workers=4))


def score_test(x,y):
  prediction = mlp(torch.tensor(x,dtype=torch.float32))
  acc = (abs(prediction.detach().numpy()-y).mean())
  minim=np.min((prediction.detach().numpy()-y))
  maxim=np.max((prediction.detach().numpy()-y))
  return acc,minim,maxim

print("score =",score_test(x_train,y_train))


print("training score=", score_test(x_train,y_train))
print("validation score=", score_test(x_test,y_test))
